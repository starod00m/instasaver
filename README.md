...
#### Instagram через HikerAPI

Публичные IP многих хостингов (в том числе через VPN) помечены Instagram как rate-limited/login-required, и yt-dlp в таких условиях не скачивает IG-ссылки. Чтобы обходить это без прокси и без собственных IG-аккаунтов, Instagram-ссылки резолвятся через коммерческий [HikerAPI](https://hikerapi.com): он возвращает прямую mp4-ссылку на CDN Meta, которую бот скачивает обычным HTTP.

**TikTok** это не касается — для TikTok остаётся прежний путь через yt-dlp + прокси.

**Как настроить:**

1. Зарегистрируйтесь на [hikerapi.com](https://hikerapi.com) и получите access key (free tier — 100 запросов).
2. Добавьте ключ в `.env`:

```env
HIKERAPI_KEY=your_hikerapi_access_key
```

Если ключа нет, бот всё равно запускается и обрабатывает TikTok. На Instagram-ссылки он отвечает сообщением про отсутствующую конфигурацию — ничего не ломает.

**Стоимость:** pay-as-you-go, $0.0006–$0.02 за запрос. Для 20–50 reels/день ожидается ~$1–5/мес.

**Что передаётся в HikerAPI:** только публичный URL поста (`instagram.com/reel/…`, `/p/…`, `/tv/…`). Никаких данных пользователя Telegram.
