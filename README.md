...
Instagram Reels скачиваются через yt-dlp без cookies и без прокси. С июня 2026 Instagram отдаёт media только клиентам с браузерным TLS-fingerprint'ом, поэтому yt-dlp использует impersonation через библиотеку [`curl_cffi`](https://github.com/lexiforest/curl_cffi). Она подтягивается автоматически как extra зависимости `yt-dlp[curl-cffi]` (см. `pyproject.toml`) — отдельная настройка не требуется.

Проверить, что impersonation доступен:

```bash
uv run yt-dlp --list-impersonate-targets
```

В выводе должны быть таргеты Chrome/Safari со `Source = curl_cffi`. Если все они помечены `(unavailable)`, `curl_cffi` не установлен и Instagram работать не будет.
...
- Работает только с **публичным** контентом Instagram и TikTok
- Telegram Bot API ограничивает размер файла до 50 МБ — yt-dlp настроен на `--max-filesize 50M` и оборвёт скачивание видео большего размера, пользователю отправляется сообщение о превышении лимита
...
- Проверьте `uv run yt-dlp --list-impersonate-targets` — если таргеты помечены `(unavailable)`, не установлен `curl_cffi`; пересоберите окружение через `uv sync`
- Если в логах `empty media response` — это общее сообщение экстрактора Instagram, оно не всегда означает отсутствие `curl_cffi`. Причиной также может быть репутация исходящего IP (дата-центр), rate-limit или geo-ограничение — в этом случае поможет смена исходящего IP/VPN, а не переустановка зависимостей
